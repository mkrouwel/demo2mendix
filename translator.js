import { BasicRelativePosition, SimpleDataType, SimpleModel } from "@trives/mxsdkcore/build/simplesdk/simplesdk.js";
import { Lib } from "@trives/mxsdkcore/build/lib/lib.js";
...
const simpleModel = new SimpleModel(config.mendixtoken);
...
const module = simpleModel.addModule(descriptor.defaultmodule + Lib.getCurrentDateTime());
    ...
    switch (factkind.type) {
        ...
        case "attributetype":
            ...
            switch (vtmapping.primitive) {
                case "string":
                    module.entities[factkind.domain].addStringAttribute(factkind.name);
                    break;
                case "datetime":
                    module.entities[factkind.domain].addDateTimeAttribute(factkind.name);
                    break;
                case "boolean":
                    module.entities[factkind.domain].addBooleanAttribute(factkind.name);
                    break;
                case "integer":
                    module.entities[factkind.domain].addIntegerAttribute(factkind.name);
                    break;
                case "decimal":
                    module.entities[factkind.domain].addDecimalAttribute(factkind.name);
                    break;
...
for (let actionrule of demomodel.actionrules) {
    // process event part...
    const newMF = mfFolder.addMicroflow(`AssessTruth_ARS${actionrule.id}_AR${actionrule.ear}`);
